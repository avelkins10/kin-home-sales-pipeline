...
export default function RepCardDataDebugTab() {
  ...
  const getAttributionBadge = (status: 'MATCH' | 'MISMATCH') => {
    if (status === 'MATCH') {
      return <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 text-xs"><CheckCircle className="w-3 h-3 mr-1 inline" />Match</Badge>;
    }
    return <Badge variant="destructive" className="bg-red-50 text-red-700 border-red-200 text-xs"><XCircle className="w-3 h-3 mr-1 inline" />Mismatch</Badge>;
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">RepCard Data Debug</h2>
                  ...
                  <tr key={closer.repcard_user_id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-2 text-center">
                      {Math.abs(closer.difference) > 5 ? (
                        ...
                        <Badge variant="outline" className="bg-green-50 text-green-700 text-xs">OK</Badge>
                      )}
                    </td>
                  </tr>
                ))}
              ...
              <tbody>
                {appointments.map((appt) => (
                  <tr key={appt.repcard_appointment_id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-2">
                      <Badge variant="outline" className="bg-green-50 text-green-700 text-xs">{appt.disposition}</Badge>
                    </td>
