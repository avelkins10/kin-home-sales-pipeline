...
// app/api/analytics/cancellation-analysis/route.ts
import { NextResponse } from 'next/server';
import { requireRole } from '@/lib/auth/guards';
import { logApiRequest, logApiResponse, logError } from '@/lib/logging/logger';
import { parseCancelReason } from '@/lib/utils/reason-parser';
 ...
 * 
 * Fetch cancellation reason analysis for analytics dashboard.
...
export async function GET(req: Request) {
  ...
  logApiRequest('GET', '/api/analytics/cancellation-analysis', undefined, reqId);
  ...
  try {
    ...
    const cached = cancellationAnalysisCache.get(cacheKey);
    const cacheTTL = 60 * 1000; // 60 seconds
    
    if (cached && Date.now() - cached.timestamp < cacheTTL) {
      logApiResponse('GET', '/api/analytics/cancellation-analysis', Date.now() - startedAt, { cached: true, timeRange: effectiveTimeRange, totalCancellations: cached.data.analysis?.length || 0 }, reqId);
      return NextResponse.json(cached.data, { status: 200 });
    }

    // Build timezone-aware date filters based on timeRange - filter by SALES_DATE within the period
    ...
    // Query cancelled projects from Quickbase
    const { qbClient } = await import('@/lib/quickbase/client');
    
    // Build where clause for Quickbase query
    const whereConditions = [`{${PROJECT_FIELDS.PROJECT_STATUS}.EX.Cancelled}`];
    ...
    const cancelledProjects = response.data || [];

    // Parse cancellation reasons with source tracking
    const reasonData = new Map<CancelReasonCategory, { count: number; sources: Map<string, number> }>();
    
    for (const project of cancelledProjects) {
      const parsedReason = parseCancelReason({
    ...
    // Calculate percentages and create analysis array
    const totalCancellations = cancelledProjects.length;
    const analysis: CancellationAnalysis[] = Array.from(reasonData.entries())
      .map(([category, data]) => {
    ...
    // Cache the result
    cancellationAnalysisCache.set(cacheKey, { data: responseData, timestamp: Date.now() });

    // Clean up old cache entries
    if (cancellationAnalysisCache.size > 50) {
      const now = Date.now();
      const entries = Array.from(cancellationAnalysisCache.entries());
      
      // Remove expired entries
      for (const [key, value] of entries) {
        if (now - value.timestamp > cacheTTL) {
          cancellationAnalysisCache.delete(key);
        }
      }
      
      // If still over 50 entries, evict oldest
      if (cancellationAnalysisCache.size > 50) {
        const remainingEntries = Array.from(cancellationAnalysisCache.entries());
        remainingEntries
          .sort((a, b) => a[1].timestamp - b[1].timestamp)
          .slice(0, cancellationAnalysisCache.size - 50)
          .forEach(([key]) => cancellationAnalysisCache.delete(key));
      }
    }

    logApiResponse('GET', '/api/analytics/cancellation-analysis', Date.now() - startedAt, { cached: false, timeRange: effectiveTimeRange, totalCancellations }, reqId);
    return NextResponse.json(responseData, { status: 200 });
  } catch (error) {
    console.error('[/api/analytics/cancellation-analysis] ERROR:', error);
    logError('Failed to fetch cancellation analysis', error as Error, {});
