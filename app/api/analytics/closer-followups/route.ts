...
import { getFollowUpsByCloser } from '@/lib/analytics/closerMetrics';
...
export async function GET(request: NextRequest) {
  ...
  try {
    logApiRequest('GET', path, { endpoint: 'closer-followups', requestId });
    ...
    const data = await getFollowUpsByCloser(timeRange, customDateRange, officeIds);
    ...
    const duration = Date.now() - start;
    logError('closer-followups', error as Error, { requestId, context: 'closer-followups' });
