...
export async function POST(request: NextRequest) {
  ...
  try {
    logApiRequest('POST /api/operations/communications/call', {}, reqId);
    ...
    const { projectId, recordId, customerPhone, customerName } = body;

    // Validate input
    if (!projectId || !customerPhone || !customerName) {
      return NextResponse.json(
        { error: 'Project ID, customer phone, and customer name are required' },
        { status: 400 }
      );
    }
    ...
    logApiResponse('POST /api/operations/communications/call', { 
    ...
    logError('POST /api/operations/communications/call', error, reqId);
