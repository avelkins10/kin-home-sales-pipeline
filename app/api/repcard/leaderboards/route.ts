...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let settersResult;
    try {
      if (hasDateFilter && hasOfficeFilter && hasTeamFilter) {
        settersResult = await sql`
          ...
          LEFT JOIN repcard_appointments a ON a.setter_user_id::int = ru.repcard_user_id
            ...
            AND (a.created_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/New_York')::date >= ${startDateParam}::date 
          ...
          AND COALESCE(ru.team, 'No Team') = ANY(${teamNames}::text[])
        ...
        `;
      } else if (hasDateFilter && hasOfficeFilter && !hasTeamFilter) {
        settersResult = await sql`
          WITH door_knock_stats AS (
            SELECT 
              dk.setter_user_id,
        ...
        settersResult = await sql`
          ...
          WHERE ru.status = 1 AND (ru.role = 'setter' OR ru.role IS NULL)
          AND (${teamNames.length === 0 ? sql`1=1` : sql`COALESCE(ru.team, 'No Team') = ANY(${teamNames}::text[])`})
